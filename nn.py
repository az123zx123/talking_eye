...
class Head(torch.nn.Module):
    ...
    def __init__(self, nc=80, filters=()):
        ...
        self.m = torch.nn.AdaptiveMaxPool2d((1,1))
        self.l = torch.nn.Sequential(
      	torch.nn.Linear(nc*3,nc),
        torch.nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            outputs.append(torch.squeeze((self.m(self.cls[i](x[i])))))
        outputs = self.l(torch.stack(outputs,dim=2).flatten(1))
        return outputs
